...
namespace xlDuckDb
{
    ...
    {
        public static object[,] ExecuteQuery(string query, string dataSource = null)
        {
                       ...
                       $"Data Source = {(string.IsNullOrEmpty(dataSource) ? ":memory:" : dataSource)}"))
            {
                ...
                using (var command = duckDbConnection.CreateCommand())
                {
                    ...
                    {
                        if (!reader.HasRows || reader.IsClosed) return new object[,] {{null}};
                        ...
                        foreach (var _ in reader)
                        {
                            ...
                            for (var i = 0; i < reader.FieldCount; i++)
                                ...
                                else if (timeSpanField[i])
                                {
                                    ...
                                    switch (tsv)
                                    {
                                        case DuckDBTimeOnly tsvDdb:
                                            rowData[i] = new DateTime(1899, 12, 30) +
                                                         TimeSpan.FromTicks(tsvDdb.Ticks);
                                            break;
                                        case TimeSpan tsvNet:
                                            rowData[i] = new DateTime(1899, 12, 30) + tsvNet;
                                            break;
                                        default:
                                            rowData[i] = tsv;
                                            break;
